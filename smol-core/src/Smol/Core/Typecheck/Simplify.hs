...
simplifyEquals ann (TLiteral _ litA) (TLiteral _ litB) = TLiteral ann (TLBool $ litA == litB)
simplifyEquals ann _ _ = TPrim ann TPBool

simplifyType :: (Show (dep Identifier), Show (dep TypeName), Show ann) => Type dep ann -> Type dep ann
