  ...
  let action = do
        ...
        let moduleVars =
              mconcat $
                ( \(modName, (hash, names)) ->
                    M.fromList
                      ( (\name -> ((name, Just modName), Dependency hash))
                          <$> S.toList names
                      )
                )
                  <$> M.toList modules
...
withLambda :: (Ord var) => Map (var, Maybe ModuleName) (Unique hash) ->
      App hash var ann a -> App hash var ann a
withLambda newVars =
